...
export default function CobrancaPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      safeApi<Pagamento[]>(router, '/billing/pagamentos', { token }).catch(() => [] as Pagamento[]),
    ])
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <MetricCard
          ...
          subtitle={
            carteira
              ? `${formatCredits(carteira.saldoPayg)} PAYG · ${formatCredits(carteira.saldoRollover)} rollover`
              : '—'
          }
        />
        <MetricCard
          ...
          subtitle={
            cotaTotal > 0
              ? `${formatCredits(cotaUsada)} / ${formatCredits(cotaTotal)} usados`
              : 'Sem cota mensal'
          }
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <div className={cn(glassCard, 'p-6')}>
          ...
          <dl className="mt-5 space-y-3 text-sm">
            <Row label="Saldo PAYG" value={formatCredits(carteira?.saldoPayg ?? 0)} />
            <Row label="Saldo rollover" value={formatCredits(carteira?.saldoRollover ?? 0)} />
          </dl>

          <p className="mt-5 text-xs text-white/35">
            Comprar créditos avulsos: em breve. Por enquanto, faça upgrade de plano para aumentar
            a cota mensal.
          </p>
        </div>
      </div>
